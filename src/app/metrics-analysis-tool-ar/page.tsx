...
export default function MetricsAnalysisToolAR() {
  ...
  const analysisTools = [
    {
      id: 'ctr-analysis',
      title: 'تحليل نسبة النقر إلى الظهور',
      description: 'تحليل نسب النقر عبر مقاييس مختلفة',
      icon: MousePointer,
    ...
    },
    {
      ...
      title: 'تحليل التحويل',
      description: 'تقييم معدلات التحويل والأداء',
      icon: Target,
    ...
    },
    {
      ...
      description: 'تفصيل التكاليف ومقاييس الكفاءة',
      icon: DollarSign,
    ...
    },
    {
      ...
      title: 'تسجيل الأداء',
      description: 'الحصول على درجات الأداء الإجمالية والتقييمات',
      icon: Gauge,
  ...
  const calculateMetrics = () => {
    ...
    if (cpm > 10) newRecommendations.push('CPM مرتفع - فكر في تحسين الاستهداف')
    if (ctrAll < 1) newRecommendations.push('نسبة نقر منخفضة - حسن الإعلان الإبداعي أو الاستهداف')
    if (conversionRate < 2) newRecommendations.push('معدل تحويل منخفض - حسن صفحة الهبوط')
    if (cpc > 5) newRecommendations.push('CPC مرتفع - راجع اختيار الكلمات المفتاحية')
    ...
    setMetrics([
      ...
      { name: 'نسبة النقر الإجمالية', value: ctrAll.toFixed(2), unit: '%', status: ctrAll > 2 ? 'good' : 'warning' },
      { name: 'نسبة النقر للرابط', value: ctrLink.toFixed(2), unit: '%', status: ctrLink > 1 ? 'good' : 'warning' },
      { name: 'نسبة النقر الفريدة', value: ctrUnique.toFixed(2), unit: '%', status: ctrUnique > 0.5 ? 'good' : 'warning' },
      { name: 'CPC', value: cpc.toFixed(2), unit: '$', status: cpc < 2 ? 'good' : 'warning' },
      { name: 'CPA', value: cpa.toFixed(2), unit: '$', status: cpa < 50 ? 'good' : 'warning' },
      { name: 'معدل التحويل', value: conversionRate.toFixed(2), unit: '%', status: conversionRate > 2 ? 'good' : 'warning' }
    ])
    setRecommendations(newRecommendations)
    setScore(scoreValue)
    setResults({
      summary: `تقرير تحليل المقاييس`,
  ...
  const downloadAnalysis = () => {
    const analysisText = `تقرير تحليل المقاييس\n\n` +
    ...
    a.href = url
    a.download = 'metrics-analysis-ar.txt'
        ...
        <div className="container mx-auto px-4 max-w-7xl">
          ...
          >
            ...
            <h1 className="text-5xl md:text-6xl font-bold mb-6 bg-gradient-to-r from-blue-400 via-cyan-400 to-purple-400 bg-clip-text text-transparent">
              تحليل مقاييس التسويق
            </h1>
            <p className="text-xl text-gray-300 max-w-3xl mx-auto leading-relaxed">
              حلل أداء حملاتك التسويقية مع تقييم شامل للمقاييس ورؤى قابلة للتنفيذ.
            </p>
          </motion.div>
          ...
          <div className="grid lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
            ...
            >
              <div className="bg-white/5 backdrop-blur-md rounded-3xl p-8 border border-white/10 shadow-2xl">
                <div className="mb-8">
                  <h2 className="text-2xl font-bold text-white mb-2">مقاييس الحملة</h2>
                  <p className="text-gray-300">أدخل بيانات أداء حملتك لتحليلها</p>
                </div>

                <div className="space-y-6">
                  <div className="grid md:grid-cols-2 gap-6">
                    <FlexibleInput
                      label="CPM (التكلفة لكل ألف ظهور)"
                      type="currency"
                      value={formData.cpm}
            ...
            >
              <div className="bg-white/5 backdrop-blur-md rounded-3xl p-6 border border-white/10 shadow-2xl">
                <h3 className="text-xl font-bold text-white mb-4">أدوات التحليل</h3>
              ...
              {score !== undefined && (
                ...
                >
                  <h3 className="text-xl font-bold text-white mb-4">درجة الأداء</h3>
                  <div className="text-center">
                    ...
                    <div className="text-sm text-gray-300">
                      {score >= 80 ? 'أداء ممتاز' : score >= 60 ? 'أداء جيد' : 'يحتاج تحسين'}
                    </div>
                  </div>
                </motion.div>
              )}
          ...
          {results && (
            ...
            >
              <EnhancedResultsDisplay
                title="نتائج تحليل الحملة"
                subtitle="تحليل شامل للمقاييس مع رؤى قابلة للتنفيذ"
                results={results}
                metrics={metrics}
          ...
          >
            <div className="text-center mb-16">
              <h2 className="text-4xl font-bold text-white mb-4">ميزات التحليل المتقدمة</h2>
              <p className="text-xl text-gray-300 max-w-3xl mx-auto">
                أدوات قوية لمساعدتك على فهم وتحسين أداء التسويق الخاص بك
              </p>
            </div>
              ...
              {[
                {
                  ...
                  description: 'راقب أداء حملتك مع تحديثات البيانات المباشرة ورؤى فورية.',
                  color: 'from-blue-500 to-cyan-500'
                },
                {
                  icon: Target,
                  title: 'تتبع التحويلات',
                  description: 'تتبع التحويلات عبر جميع القنوات وحسّن للحصول على نتائج أفضل.',
                  color: 'from-green-500 to-emerald-500'
                },
                {
                  icon: TrendingUp,
                  title: 'اتجاهات الأداء',
                  description: 'حدد الاتجاهات والأنماط في بيانات حملتك بمرور الوقت.',
                  color: 'from-purple-500 to-pink-500'
                },
                {
                  icon: Zap,
                  title: 'رؤى ذكية',
                  description: 'احصل على توصيات مدعومة بالذكاء الاصطناعي لتحسين استراتيجية التسويق الخاصة بك.',
                  color: 'from-orange-500 to-red-500'
                }
              ].map((feature, index) => (
                <motion.div
